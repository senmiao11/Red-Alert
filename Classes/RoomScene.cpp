...
void RoomScene::popupChooseLayer()
{
	//根据背景创建对话框
	...
	//设置对话框的按键相应方法
	popDialog->setCallbackFunc(this, callfuncN_selector(RoomScene::chooseMapCallback));
	this->addChild(popDialog);
}

void RoomScene::chooseMapCallback(Node* pNode)
{
	if (pNode->getTag() == 1)
	{
		//选择地图一
